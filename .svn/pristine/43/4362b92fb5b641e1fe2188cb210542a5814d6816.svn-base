		...
		<div class="map">
			<div class="panel panel-default" id="map1" :class="{dn:!mapOrBind}">
				<div class="heading clearfix">
					...
					<div class="btns fr">
						...
						当前场区：
							<input type="text" style="color:#000;" id="currentarea" readonly="readonly" v-model="mergeName">
						</span>
			...
			<div class="content panel panel-default" id="bindcontent" :class="{dn:mapOrBind}">
    			...
    			<div class="plist" id="bindlist">
					...
					<div v-for="(item,index) in spaceEquipList" :key="index">
						<div class="kind clearfix">
							<span>{{item.typeName}}</span>
							<button type="button" class="fr btn btn-primary" @click="spaceBindEquip(item.typeCode)" :disabled="item.flag==true">绑定</button>
						</div>
						<div>
							<ul class="klist">
								<li class="clearfix" :key="itemSub.devId" v-for="(itemSub) in spaceBindedList">
									<div v-if="itemSub.onerankdevType==item.typeCode">
										<span>设备ID：{{itemSub.onerankdevTerminId}}，设备SN：{{itemSub.onerankdevDevSn}}</span>
		      	...
		      	<el-form-item label="布局底图" prop="bgImgUrl">
		    		<el-col :span="18">
		      		...
		      		<el-col :span="5" style="margin-left:6px;">
		      			<el-upload
						  class="upload-demo"
						  action="http://192.168.1.113:9992/oss/upload"
				...
				<el-form-item label="SN" prop="sn">
		    		<el-col :span="18">
		      			<el-select v-model="formEquip.sn" placeholder="请选择" style="width:100%;">
						    <el-option
						      v-for="item in spaceUseEquip"
						      :key="item.index"
						      :label="item.onerankdevTerminId"
	...
	export default {
		...
		methods:{
			...
			spaceBindEquip:function(typeCode){
				...
				console.log(this.currentComp,1);
			...
			editIo:function(node,data){
				if(data.level==1){
					...
					this.$postRequest(getIoSingle(),{parkingIoId:data.id}).then((data) => {
						...
						this.formIo.version=data.data.version;
			...
			handleMousedown:function(e){
				...
				if(_name=="DIV"){
					...
					component_type = this.controlMouse.tdClass.split("_")[2]
					if(component_type=='2'){
						id = this.controlMouse.tdClass.split("_")[4].split(' ')[0];
						// a = (component_data[4] + ' ').split(' ')[0];
						this.$postRequest(getIoSingle(),{parkingIoId:id}).then((data) => {
							if(data.status!=200) return;
							this.version = data.data.version
		...
		computed:{
			...
			mergeName(){
				return this.parkInfo.parkingName+'-'+this.areaInfo.name
			} 
		},
		created:function(){
			this.permiss;
